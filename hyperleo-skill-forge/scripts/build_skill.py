...
MAX_DESC_TRIGGERS = 8
MIN_TRIGGERS = 6
HYPERLEO_NAME = re.compile(r"^hyperleo(?:-[a-z0-9]+){2,3}$")
...
def validate_spec(spec: dict) -> list[str]:
    ...
    if not HYPERLEO_NAME.fullmatch(name):
        errors.append(f"name 不符合 HyperLeo 命名规范（hyperleo-单词-单词，最多 3 个单词）：{name!r}")
