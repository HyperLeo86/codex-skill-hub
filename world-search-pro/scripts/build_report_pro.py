...
def rec_card_html(candidate):
    ...
    notes = []
    if candidate.get("verdict_line"):
        notes.append(f"🎯 决策行：{esc(candidate['verdict_line'])}")
    if candidate.get("known_issues"):
        notes.append(f"⚠️ 已知坑：{esc(candidate['known_issues'])}")
    if candidate.get("license_check"):
        notes.append(f"⚖️ 许可硬检查：{esc(candidate['license_check'])}")
    if candidate.get("confidence") or candidate.get("door"):
        door = candidate.get("door", "—")
        notes.append(f"🧭 置信度：{esc(candidate.get('confidence', '—'))} · 门类型：{esc(door)}")
    if candidate.get("re_evaluation_trigger"):
        notes.append(f"🔁 再评估触发器：{esc(candidate['re_evaluation_trigger'])}")
    if candidate.get("lineage"):
        notes.append(f"📜 谱系：{esc(candidate['lineage'])}")
...
<div class="wrap">
  ...
  <div class="section"><h2>Pro 决策摘要</h2>{{PRO_SUMMARY}}</div>
  ...
  <div class="footer">由 检索世界 Pro 生成 · <a href="{{INDEX_LINK}}">返回检索历史</a></div>
</div>
</body>
</html>"""
...
def render_report(report, history_dir, run_dir):
    ...
    pro_lines = []
    if report.get("verdict_line"):
        pro_lines.append(f"<p><b>Verdict 行</b>：{esc(report['verdict_line'])}</p>")
    if report.get("confidence"):
        door = report.get("door", "—")
        pro_lines.append(f"<p><b>置信度</b>：{esc(report['confidence'])} · <b>门类型</b>：{esc(door)}</p>")
    if report.get("re_evaluation_trigger"):
        pro_lines.append(f"<p><b>再评估触发器</b>：{esc(report['re_evaluation_trigger'])}</p>")
    if report.get("lineage"):
        pro_lines.append(f"<p><b>谱系</b>：{esc(report['lineage'])}</p>")
    if report.get("business_value"):
        pro_lines.append(f"<p><b>业务价值五维</b>：{esc(report['business_value'])}</p>")
    if not pro_lines:
        pro_lines.append("<p>本报告未启用 Pro 决策字段（可运行 world-search-pro 完整流程生成）。</p>")
    pro_summary = f'<div class="card">{"".join(pro_lines)}</div>'
    ...
    return (
        ...
        .replace("{{PRO_SUMMARY}}", pro_summary)
